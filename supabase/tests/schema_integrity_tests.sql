...
-- =============================================================================
-- MOVED OUT: the checks that read production rather than fixtures
-- =============================================================================
-- Three sections used to sit here -- SECURITY INVOKER triggers reaching into
-- app, a customer with a live loan outside every active area, a village whose
-- state the PIN directory does not carry. They are in live_invariants_tests.sql
-- now, which is `@target: production` and runs read-only.
--
-- They were the only assertions in this file that read the real book instead of
-- the fixtures above, and this file can never run against a real book: it
-- fabricates persons and businesses, so the runner refuses it anywhere that
-- holds any. Left here they would only ever have been evaluated against an
