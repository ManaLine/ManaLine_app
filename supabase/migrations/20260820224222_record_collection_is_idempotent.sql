-- record_collection replays instead of double-recording.
--
-- An Agent on 2G taps Save, the reply never arrives, they press Retry — and
-- every retry used to record a SECOND collection: the customer's balance down
-- twice, the agent's float up twice, invisible until a settlement failed. The
-- duplicate WARNING already here catches two different agents colliding, not
-- one agent's own request being retried.
--
-- p_idempotency_key appended so every existing caller is untouched, and the
-- previous signature dropped so PostgREST is not left choosing between two
-- overloads. (Third time this trap has come up this week - the DROP is not
-- optional.)
...
BEGIN
  ...
  IF NOT p_confirm_duplicate THEN
    IF EXISTS (
      ...
      -- Not stored against the key: nothing was written, and the Agent may
