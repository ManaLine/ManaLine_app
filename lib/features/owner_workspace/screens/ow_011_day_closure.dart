    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (widget.openForReopen) {
        ref
            ...
            .loadForReopen(businessId: widget.businessId, businessDate: widget.businessDate);
      } else {
        ref
            .read(dayClosureProvider.notifier)
            .runPrecheck(businessId: widget.businessId, businessDate: widget.businessDate);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        Text('Day Closure Cannot Start', style: Theme.of(context).textTheme.headlineMedium),
        ...
        ...state.blockingIssues.map((issue) => Card(
              child: ListTile(
                leading: const Icon(Icons.error_outline, color: ManaColors.statusBad),
        ...
        if (state.warnings.isNotEmpty) ...[
          const SizedBox(height: ManaSpacing.lg),
          const ManaText('warnings (may proceed)', style: TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: ManaSpacing.xs),
          ...state.warnings.map((w) => Card(
                color: ManaColors.statusWarnFaint,
                child: ListTile(
                  leading: const Icon(Icons.warning_amber, color: ManaColors.statusWarn),
...
class _CashVerificationState extends ConsumerState<_CashVerification> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      padding: const EdgeInsets.all(ManaSpacing.lg),
      children: [
        Text('Cash Verification', style: Theme.of(context).textTheme.headlineMedium),
        const SizedBox(height: ManaSpacing.xs),
        const ManaText.raw(
        ...
        const SizedBox(height: ManaSpacing.lg),
        _AmountField(label: 'Physical Cash', controller: _cash, onChanged: _onChanged),
        _AmountField(label: 'UPI Balance', controller: _upi, onChanged: _onChanged),
        _AmountField(label: 'Bank Balance', controller: _bank, onChanged: _onChanged),
        _AmountField(label: 'Cheque Balance', controller: _cheque, onChanged: _onChanged),
        const SizedBox(height: ManaSpacing.lg),
        if (expected != null) ...[
          const ManaText('system expected (computed)', style: TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: ManaSpacing.sm),
          ...
          _ExpectedRow(label: 'Expected Cheque', value: expected.expectedCheque),
        ],
...
class _AmountField extends StatelessWidget {
  ...
  const _AmountField({required this.label, required this.controller, required this.onChanged});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ManaText.raw(label, style: const TextStyle(color: ManaColors.textSecondary)),
          Text(_currency.format(value), style: Theme.of(context).textTheme.bodyMedium),
        ],
      ),
    );
  }
...
class _DifferenceAnalyzer extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_DifferenceAnalyzer> createState() => _DifferenceAnalyzerState();
}
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: ManaSpacing.sm),
        Text('Difference Found', style: Theme.of(context).textTheme.headlineMedium),
        const SizedBox(height: ManaSpacing.xs),
        Text(
          'Overall Difference: ${_currency.format(state.difference)}',
          style: Theme.of(context).textTheme.titleLarge?.copyWith(color: ManaColors.statusBad),
        ),
        const SizedBox(height: ManaSpacing.lg),
        const ManaText('difference details', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: ManaSpacing.sm),
        ...state.differenceLines.map((l) => Card(
              child: ListTile(
                title: ManaText.raw(l.method),
                subtitle: Text('Expected ${_currency.format(l.expected)} · Actual ${_currency.format(l.actual)}'),
                trailing: Text(
                  _currency.format(l.delta),
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color: l.delta == 0 ? ManaColors.statusGood : ManaColors.statusBad,
                  ),
                ),
              ),
            )),
        ...
        if (state.recordedAdjustments.isNotEmpty) ...[
          const ManaText('adjustments recorded this session', style: TextStyle(fontWeight: FontWeight.bold)),
          ...state.recordedAdjustments.map((a) => ListTile(
                leading: Icon(
                  a.adjustmentType == 'Short' ? Icons.arrow_downward : Icons.arrow_upward,
                  color: a.adjustmentType == 'Short' ? ManaColors.statusBad : ManaColors.statusWarn,
                ),
                title: Text('${a.adjustmentType} · ${_currency.format(a.amount)}'),
...
class _RecordAdjustmentDialog extends StatefulWidget {
  ...
  @override
  State<_RecordAdjustmentDialog> createState() => _RecordAdjustmentDialogState();
}
...
class _RecordAdjustmentDialogState extends State<_RecordAdjustmentDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final valid = amount > 0 && (!needsCustomer || _targetCustomerId.text.trim().isNotEmpty);
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: _appliedToOptions.map((o) => DropdownMenuItem(value: o, child: ManaText.raw(o))).toList(),
            ...
            if (needsCustomer) ...[
              ...
              TextField(
                ...
                decoration: const InputDecoration(labelText: 'Target Customer ID'),
                onChanged: (_) => setState(() {}),
              ),
            ],
            const SizedBox(height: ManaSpacing.md),
        ...
        FilledButton(
          onPressed: valid
              ? () => Navigator.of(context).pop(_AdjustmentInput(
                    ...
                    targetCustomerId: needsCustomer ? _targetCustomerId.text.trim() : null,
  ...
  Future<void> _confirm() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.of(context).pop(true), child: const ManaText('confirm close')),
        ],
      ),
    );
    ...
    if (ok != true && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Difference is no longer zero — recheck required.')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: ManaSpacing.lg),
        _SummaryRow(label: 'Opening Balance', value: 0), // TODO: from day_ledger, deferred to API pass
        if (expected != null) _SummaryRow(label: 'Collections', value: expected.expectedCash),
        _SummaryRow(label: 'Adjustments', value: state.recordedAdjustments.fold(0.0, (a, b) => a + b.amount)),
        const Divider(),
        _SummaryRow(label: 'Closing Balance', value: state.actualTotal, bold: true),
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            const ManaText('Difference', style: TextStyle(fontWeight: FontWeight.bold)),
            const ManaStatusPill(label: '₹0.00 — Balanced', status: ManaStatus.good),
          ],
        ),
        const SizedBox(height: ManaSpacing.lg),
        TextField(
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          ManaText(label, style: bold ? const TextStyle(fontWeight: FontWeight.bold) : null),
          Text(_currency.format(value),
              style: bold ? Theme.of(context).textTheme.titleMedium : Theme.of(context).textTheme.bodyMedium),
        ],
      ),
    );
  }
...
class _ClosedReceipt extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: ManaSpacing.sm),
        Text('Business Day Closed', style: Theme.of(context).textTheme.headlineMedium),
        const SizedBox(height: ManaSpacing.xs),
        ManaText.raw('${detail.businessDate} · Closed by ${detail.closedByName}',
        ...
        _SummaryRow(label: 'Deposits (Investor)', value: detail.depositsInvestor),
        _SummaryRow(label: 'Withdrawals (Investor)', value: detail.withdrawalsInvestor),
        _SummaryRow(label: 'Adjustments', value: detail.adjustments),
        const Divider(),
        _SummaryRow(label: 'Closing Balance', value: detail.closingBalance, bold: true),
        if (detail.remarks != null) ...[
          const SizedBox(height: ManaSpacing.md),
          ManaText.raw('Remarks: ${detail.remarks}', style: const TextStyle(color: ManaColors.textSecondary)),
        ],
  ...
  Future<void> _openReopenDialog(BuildContext context, WidgetRef ref) async {
    ...
    await NetworkErrorHandler.run(context, () async {
      return ref.read(dayClosureProvider.notifier).reopenClosedDay(reason: reason);
    });
  }
}
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: ManaSpacing.xxl),
        FilledButton(
          onPressed: () => ref.read(dayClosureProvider.notifier).closeAgain(businessId: businessId),
