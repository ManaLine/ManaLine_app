-- Definitive app.migrate_loan. Full CREATE OR REPLACE, so this file alone
-- determines the final function regardless of the three superseded steps
-- before it.
--
-- loans.live_photo_url is NOT NULL — BR-036/BR-081 ("live photo mandatory
-- for every new loan") is enforced by the schema, not just by the UI.
--
-- A pre-existing loan has no live photo and never will: the customer was
-- not standing in front of anyone when the Owner copied the row out of a
-- notebook. Rather than making the column nullable — which would silently
-- waive the rule for REAL loans too — migrated rows carry an explicit
-- sentinel. It is deliberately not a URL, so nothing can mistake it for a
-- real photo, and it is greppable if these rows ever need auditing.
--
-- Verified end to end against the Owner's own worked example (executed as
-- the authenticated owner role, then rolled back): investment 10,00,000,
-- given 40,000, repayment 40,000, remaining 25,000 ->
--   collected 15,000, interest 0, BF 9,75,000, line balance 25,000,
--   13 weekly instalments starting today summing to exactly 25,000
--   (12 x 2,000 plus a 1,000 stub).
CREATE OR REPLACE FUNCTION app.migrate_loan(
  ...
  p_collection_agent_membership_id uuid DEFAULT NULL
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- collection_agent_membership_id is NOT NULL: every loan belongs to
  -- someone who collects it. Resolve rather than make the Owner restate it
  -- per row.
  ...
  -- amount_given is a GENERATED column (repayment - interest - fee), so it
  -- is set implicitly by interest_amount. BR-004/BR-011 is enforced by the
  -- schema itself.
  INSERT INTO loans (
    ...
    v_status,
    -- The ORIGINAL date, not today: refresh_day_ledger sums amount_given by
    -- issue_business_date, and dating a migrated book today would report the
    -- whole old portfolio as today's loan distribution.
    p_effective_date,
    'migrated:pre-existing-loan:no-live-photo'
  ) RETURNING loan_id INTO v_loan_id;

  -- Forward-only schedule. Nothing is written for the past.
  v_left := v_remain;
  v_due := CURRENT_DATE;
  i := 1;
  WHILE v_left > 0 LOOP
  ...
  -- BF is cash in hand: the money went out, some came back.
  UPDATE businesses
  SET owner_bf_balance = owner_bf_balance - v_given + v_collected
