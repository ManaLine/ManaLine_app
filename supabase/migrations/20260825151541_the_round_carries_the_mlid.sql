-- The round carries the MLID.
--
-- The collection row showed the loan number, which is a system identifier
-- nobody says out loud. What is actually used at a door is the MANA LINE ID:
-- it is on the customer's own card, and it is how two customers of the same
-- name in the same village are told apart before any money changes hands.
--
-- CREATE OR REPLACE, not DROP: appending a trailing column is allowed in
-- place, and dropping the view would take its grants with it.
