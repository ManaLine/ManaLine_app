...
/// CW-006 My Profile / Business Memberships — real Supabase wiring,
/// direct Customer-side counterpart of IW-005 (identical persons/
/// person_addresses/business_members query shapes; per IW-005's own API
/// BINDING note, no reinvention needed here).
///
...
/// value in history and notify every Owner this Customer is connected
...
/// OW-004 Customer Management) can correct it.
class CustomerProfileApiService {
  ...
  // Cross-business, cross-role membership list — separate call from
  // the profile fetch, per CW-006's own API BINDING. Maps to
  // GET /persons/{person_id}/memberships (04_API_Specification_v1_Part1.md
  // §2.10) via `business_members` joined to `businesses`.
  Future<List<CustomerBusinessMembership>> fetchMemberships({required String personId}) async {
    ...
    return (rows as List).cast<Map<String, dynamic>>().map((r) {
      ...
      return CustomerBusinessMembership(
  ...
  /// invariant), same fix as the Investor-side equivalent.
  ///
  ...
  /// schema doc itself describes and that `customer_state.dart`'s own
  /// `createCustomer` already uses for the first-address-ever case: the
  /// previous `is_current=true` row is closed out (`to_date`,
  ...
  /// (Addendum item 6 does add a 'Customer Address Updated'
  /// notification_type value to the enum — but the row itself must still
  /// be system-written, not client-written, per the RLS deny-all-insert
  /// policy.)
  ...
  /// up from `locations` via villageId, same fix as the Investor-side
  /// equivalent. villageName kept for display use in the picker UI.
...
final customerProfileApiServiceProvider = Provider<CustomerProfileApiService>((ref) {
  return CustomerProfileApiService();
});
...
class CustomerProfileState {
  ...
  final List<CustomerBusinessMembership> memberships;
  final String? error;
...
class CustomerProfileNotifier extends Notifier<CustomerProfileState> {
  ...
  Future<bool> updatePhone({required String personId, required String phoneNumber}) async {
    try {
      await ref.read(customerProfileApiServiceProvider).updatePhone(personId: personId, phoneNumber: phoneNumber);
  ...
  Future<bool> updateAddress({
    ...
    try {
      await ref.read(customerProfileApiServiceProvider).updateAddress(
            personId: personId,
            addressId: addressId,
            villageId: villageId,
...
final customerProfileProvider = NotifierProvider<CustomerProfileNotifier, CustomerProfileState>(
