...
class _BusinessMigrationScreenState extends ConsumerState<BusinessMigrationScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // "add a customer" rather than "add existing loan": the form now takes
      // the person and their loan together, and the person is the part an
      // Owner is thinking about when they open this screen.
      floatingActionButton: (s != null && !s.migrationLocked)
          ? FloatingActionButton.extended(
              onPressed: _addLoan,
              icon: const Icon(Icons.person_add_alt_1),
              label: const ManaText('add a customer'),
            )
          : null,
      body: SafeArea(
        child: _loading
...
class _MigrateLoanScreenState extends ConsumerState<_MigrateLoanScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const ManaText('add a customer')),
      body: SafeArea(
        child: ListView(
          padding: const EdgeInsets.all(ManaSpacing.lg),
          children: [
            ...
            SegmentedButton<bool>(
              segments: const [
                ButtonSegment(value: false, label: ManaText('existing customer')),
                ButtonSegment(value: true, label: ManaText('new person')),
              ],
              selected: {_newPerson},
